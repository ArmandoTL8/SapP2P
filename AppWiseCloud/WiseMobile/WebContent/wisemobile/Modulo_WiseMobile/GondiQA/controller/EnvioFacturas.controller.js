...
sap.ui.define(["jquery.sap.global", "sap/ui/core/mvc/Controller", "sap/m/MessageBox", "sap/ui/model/Filter", "sap/ui/model/FilterOperator", "sap/m/TablePersoController"], 
	...
	return Controller.extend("com.axiomasoluciones.wisemobile.Modulo_WiseMobile.GondiQA.controller.EnvioFacturas", {
		...
		// simulate end of operation
			...
			/*var oBusyDialog_c = new sap.m.BusyDialog({
				...
				close : function(oEvt) {
					...
					oCnt_FHelps.f_showMessage("INFORMATION", oCnt_FHelps.f_readTranslate("EnvioFacturas.1.Exito.text"), undefined, oCnt_FHelps.f_readTranslate("Exito.title") );
					oView.byId("idTableEnvioFacturas").setFooterText("Total: $0.00");
		...
		if(that.Table_Documentos == undefined)
						...
						new sap.m.Button({
							...
							press: function(oEvt){
							    ...
							    if(length==0){
					        		...
					        		oView.byId("idTableEnvioFacturas").setFooterText("Total: $" + parseFloat(total).toFixed(2) );
					        		sap.ui.getCore().getModel("mTables").refresh();
				...
				template : 
					new sap.m.ColumnListItem({
						...
						cells : [
							...
							new sap.m.Text({ text : "{mTables>CtdAut}" }),
							new sap.m.Text({ text : "{mTables>CtdRec}" }),
							new sap.m.Text({ text : "{mTables>PO_UNIT}" }),
							new sap.m.Text({ text : "{mTables>NETWR}" }),
			...
			var oDialog = new sap.m.Dialog("idPageDialog", {
				...
				beforeOpen: function(oEvt){
					...
					//oC_Modulo_WiseMobile.f_createFilterPanel("ORDENCOMPRACFACTURA_2", that, "idPageDialog", 4);
		...
		f_createAllFiltersPanel: function(oEvt){
			oC_Modulo_WiseMobile.f_createFilterPanel("ORDENCOMPRACFACTURA_2", this, "idPageDialog", 1);
		},
		
		f_bindTable: function(Parameters){/*
			var oTable = oCore.byId("idTableDocumentos");
		...
		oServ_SelecDoc: function(pParameters) {
	        ...
	        function OnSuccess(data, status)
	        {
	            ...
	            var oObjeto=sap.ui.getCore().getModel("mMessages").getProperty("/env:Envelope/env:Body/n0:ZW18_PO_PENDING_INVResponse/ES_RETURN");
	            
	            if(oObjeto.TYPE == "S"){

	            	//var oTable = oCore.byId("idTableDocumentos");

		            var oItems = sap.ui.getCore().getModel("mMessages").getProperty("/env:Envelope/env:Body/n0:ZW18_PO_PENDING_INVResponse/ET_PO/item");
		...
		onChangeArea: function(oEvt){
			...
			for(i=0;i<length;i++){
				...
				if(mData[i].IdDireccion==IdDireccion){
		...
		oServ_Impuestos_ZW18 : function() {
			var soapMessage = '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:sap-com:document:sap:rfc:functions">';
				soapMessage=soapMessage+'<soap:Header/>';
				soapMessage=soapMessage+'<soap:Body>';
				soapMessage=soapMessage+'<urn:ZW18_GET_TAX_LIST>';
	        ...
	        function OnSuccess(data, status)
	        {
	            ...
	            var length=sap.ui.getCore().getModel("mCombos").getProperty("/IMPUESTOS_ZW18/env:Envelope/env:Body/n0:ZW18_GET_TAX_LISTResponse/ET_TAX_LIST/item").length;
	            
	            if(length==undefined){
	            	
	            	var mData=sap.ui.getCore().getModel("mCombos").getProperty("/IMPUESTOS_ZW18/env:Envelope/env:Body/n0:ZW18_GET_TAX_LISTResponse/ET_TAX_LIST/item");
		...
		oServ_AreaResp_ZW18 : function(ExBukrs) {
			var soapMessage = '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:sap-com:document:sap:soap:functions:mc-style">';
				soapMessage=soapMessage+'<soap:Header/>';
				soapMessage=soapMessage+'<soap:Body>';
				soapMessage=soapMessage+'<urn:Zw18FmWsAreaResp>';			         
				soapMessage=soapMessage+'<ExBukrs>'+ExBukrs+'</ExBukrs>';
				soapMessage=soapMessage+'</urn:Zw18FmWsAreaResp>';
				soapMessage=soapMessage+'</soap:Body>';
				soapMessage=soapMessage+'</soap:Envelope>';
	        ...
	        function OnSuccess(data, status)
	        {
	        	...
	        	var mData=sap.ui.getCore().getModel("mCombos").getProperty("/AREA_RESP_ZW18/env:Envelope/env:Body/n0:Zw18FmWsAreaRespResponse/ImGerencia/item");
	        	var length=mData.length;	            
	            	...
	            	for(i=0;i<length;i++){
	            		...
	            		if(i==0){
	            			...
	            			for(j=0;j<data.length;j++){
		            			...
		            			if(mData[i].IdDireccion==data[j].IdDireccion){
		            				cont=true;
		            				break;
		            			}
			            	}
		...
		oServ_Concepto_ZW18 : function(ExBukrs) {
			var soapMessage = '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:sap-com:document:sap:soap:functions:mc-style">';
				soapMessage=soapMessage+'<soap:Header/>';
				soapMessage=soapMessage+'<soap:Body>';
				soapMessage=soapMessage+'<urn:Zw18FmWsListConceptos>';			         
				soapMessage=soapMessage+'<ExBukrs>'+ExBukrs+'</ExBukrs>';
				soapMessage=soapMessage+'</urn:Zw18FmWsListConceptos>';
				soapMessage=soapMessage+'</soap:Body>';
				soapMessage=soapMessage+'</soap:Envelope>';
	        ...
	        function OnSuccess(data, status)
	        {
	        	...
	        	var mData=sap.ui.getCore().getModel("mCombos").getProperty("/CONCEPTO_ZW18/env:Envelope/env:Body/n0:Zw18FmWsListConceptosResponse/ImListConceptos/item");
	        	var length=mData.length;	            
		...
		oServ_EnviarFectSOC_ZW18 : function(oObject,callback) {
			...
			var soapMessage = '<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:sap-com:document:sap:soap:functions:mc-style">';
				soapMessage=soapMessage+'<soap:Header/>';
				soapMessage=soapMessage+'<soap:Body>';
				soapMessage=soapMessage+'<urn:Zw18FmWsCreateIncinvWoPo>';
				soapMessage=soapMessage+'<ExCreateIncinvIn>';
				soapMessage=soapMessage+'<Bukrs>'+oObject.Bukrs+'</Bukrs>';
				//soapMessage=soapMessage+'<Ekorg/>';
				soapMessage=soapMessage+'<Lifnr>'+Lifnr+'</Lifnr>';
				soapMessage=soapMessage+'<IdDireccion>'+oObject.IdDireccion+'</IdDireccion>';
				soapMessage=soapMessage+'<IdGerencia>'+oObject.IdGerencia+'</IdGerencia>';
				soapMessage=soapMessage+'<Factura>'+oObject.Factura+'</Factura>';
				
				//soapMessage=soapMessage+'<Tax>0.00</Tax>';
				
				soapMessage=soapMessage+'<PdfXstring>'+oObject.PdfXstring+'</PdfXstring>';
				soapMessage=soapMessage+'<OpdfXstring>'+oObject.OpdfXstring+'</OpdfXstring>';
				if(_oFlag_Key == "4"){
					...
					soapMessage=soapMessage+'<TaxCode>'+oObject.TaxCode+'</TaxCode>';
				}
				if(_oFlag_Key == "3"){
					soapMessage=soapMessage+'<Flag>1</Flag>';
					soapMessage=soapMessage+'<XmlXstring>'+oObject.XmlXstring+'</XmlXstring>';
				}
				
				//soapMessage=soapMessage+'<Reten>0.00</Reten>';
				soapMessage=soapMessage+'<IdConcepto>'+oObject.IdConcepto+'</IdConcepto>';
